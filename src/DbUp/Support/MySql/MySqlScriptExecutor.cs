...
namespace DbUp.Support.MySql
{
    ...
    public sealed class MySqlScriptExecutor : IScriptExecutor
    {
        ...
        private static IEnumerable<string> SplitByGoStatements(string script)
        {
            ...
            var statements = new List<string> {"DELIMITER GO"};
            statements.AddRange(scriptStatements);
        ...
        public void VerifySchema()
        {
            ...
            sqlRunner.ExecuteNonQuery(string.Format(
                @"IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = N'{0}') Exec('CREATE DATABASE {0}')", Schema));
        }
        ...
        public void Execute(SqlScript script, IDictionary<string, string> variables)
        {
            ...
            log().WriteInformation("Executing MySql Server script '{0}'", script.Name);
